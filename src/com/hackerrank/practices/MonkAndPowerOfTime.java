...
public class MonkAndPowerOfTime {
    /**
     * Problem link - https://www.hackerearth.com/practice/data-structures/arrays/1-d/practice-problems/algorithm/monk-and-power-of-time/
     * <p>
    ...
    public static void startSolution() throws IOException {
        ...
        String[] callingOrderArray = br.readLine().split(" ");
        String[] idealOrderArray = br.readLine().split(" ");
        int i = 0;
        int count = 0;
        int matchedCount = 0;
        while (i < size) {
            if (callingOrderArray[i].equals(idealOrderArray[i])) {
                i++;
                matchedCount++;
                count++;
            } else {
                shiftArrayByOne(callingOrderArray, matchedCount);
                count++;
            }
        }
    ...
    private static void shiftArrayByOne(String[] arr, int count) {
